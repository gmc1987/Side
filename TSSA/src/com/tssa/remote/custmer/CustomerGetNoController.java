...
//		2、计算出不同出号规则所有记录状态等于1的记录进行统计，作为前面等待人数输出显示
	...
	public Map<String, Object> checkCustHasTakeNumber(String custId, String typeId){
		...
		try {
			...
			if(set != null) {
				DetachedCriteriaTS<AllocateSeatNumberRecord> detachedCriteria = new DetachedCriteriaTS<AllocateSeatNumberRecord>(AllocateSeatNumberRecord.class);
				...
				detachedCriteria.add(Restrictions.between("createDate", DateWarpUtils.parseDate(DateWarpUtils.format(new Date()) + " 00:00:00"), DateWarpUtils.parseDate(DateWarpUtils.format(new Date()) + " 23:59:59")));
