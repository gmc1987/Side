	...
	//					autoLoad:true,  
				  	...
				  	addBtn.on('click', function(){
				  		if(selectForm.getSelectionModel().hasSelection()){
							...
							aForm.findField('cooperationId').setValue(cooperationId);
							addSelectWin.hide();
				  		}else{
	...
	//					autoLoad:true,  
				  	...
				  	addBtn.on('click', function(){
				  		if(selectForm.getSelectionModel().hasSelection()){
							var record = selectForm.getSelectionModel().getSelected();
							var cooperationId = record.get("cooperId");
							mForm.findField('cooperationId').setValue(cooperationId);
							modSelectWin.hide();
				  		}else{
							Ext.MessageBox.alert("错误","没有选中修改项");
							return;
	...
	function search(){
		...
		gridForm.getStore().load({
			proxy : new Ext.data.HttpProxy({
				url : "${ctx}/user/list.do",//获取数据的后台地址
			}),
			params:{
				userCode : code,
				userName : name,
	...
	modBtn.on('click', function(){
		if(gridForm.getSelectionModel().hasSelection()){
			...
			modWin.on('show', function() {
				...
				mForm.findField('userName').setValue(userName);
				mForm.findField('userCode').setValue(userCode);
