...
Ext.onReady(function(){
	...
	myToolbar.doLayout(); 
  	var orderStore = new Ext.data.JsonStore({ 
 		url:'${ctx}/restaurantOrder/list.do',
		autoLoad:true,  
	...
	/*产品优惠价*/
  	...
  	function orderStatusMapping(value, cellmeta, record,rowIndex,columnIndex,store){
  		switch(value){
  			case 0:
  				return "新建";
  				break;
 	...
 	var executingBtn = new Ext.Button({
		text : '执行订单',
		cls: 'x-btn-text-icon scroll-bottom', 
		icon: '${img_path_16}/categories/applications-development.png',
	});
 	
 	var settlementBtn = new Ext.Button({
		text : '结算订单',
		cls: 'x-btn-text-icon scroll-bottom', 
		icon: '${img_path_22}/actions/dialog-finish.png',
	});
	...
	var gridForm = new Ext.grid.GridPanel({
		...
		tbar : [executingBtn, "-", settlementBtn],
	...
	//执行订单
	executingBtn.on('click', function(){
 		if(gridForm.getSelectionModel().hasSelection()){
			Ext.MessageBox.confirm('提示', '确认要执行该订单吗?', executing);
			function executing(btn){
				if(btn == 'yes'){
					Ext.MessageBox.wait("数据正在处理中,请稍後...", "执行");
					var record = gridForm.getSelectionModel().getSelected();
					var id = record.get('orderId');
					Ext.Ajax.request({
						url : '${ctx}/restaurantOrder/orderStatusChange.do',
						method : 'POST',
						params: { orderId : id, changeType:0 },
						success : function(response,request){
							var jsonObj = eval('(' + response.responseText + ')');
							Ext.MessageBox.hide();
							Ext.MessageBox.alert("提示",jsonObj.msg);
							gridForm.getStore().reload();
						},
			...
			Ext.MessageBox.alert("错误","没有选中执行项");
			return;
		}
	});
	...
	settlementBtn.on('click', function(){
 		if(gridForm.getSelectionModel().hasSelection()){
			Ext.MessageBox.confirm('提示', '确认要结束该订单吗?', executing);
			function executing(btn){
				if(btn == 'yes'){
					Ext.MessageBox.wait("数据正在处理中,请稍後...", "结束");
					var record = gridForm.getSelectionModel().getSelected();
					var id = record.get('orderId');
					Ext.Ajax.request({
						url : '${ctx}/restaurantOrder/orderStatusChange.do',
						method : 'POST',
						params: { orderId : id, changeType:1 },
						success : function(response,request){
							var jsonObj = eval('(' + response.responseText + ')');
							Ext.MessageBox.hide();
							Ext.MessageBox.alert("提示",jsonObj.msg);
